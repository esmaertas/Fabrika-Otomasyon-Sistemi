    ...
    public partial class frmMakineler : Form
    {
        ...
        private void btnKaydet_Click(object sender, EventArgs e)
        {
            if (btnKaydet.Text=="KAYDET" && txtMakineAdi.Text != "" && cmbBolum.SelectedIndex != -1)
            {
                ...
                sqCom.CommandText =
                    "INSERT INTO bakMakineler (MakineAdi, BolumID, Secenek) VALUES (@MakineAdi, @BolumID, @Secenek)";
            ...
            else if (btnKaydet.Text=="GÜNCELLE")
            {
                if (btnKaydet.Text=="GÜNCELLE" && txtMakineAdi.Text != ""  && cmbBolum.SelectedIndex != -1)
                {
                    if (!String.IsNullOrEmpty(lblMakineID.Text))
                    {
                        ...
                        sqCom.Connection = sqlConn;
                        sqCom.CommandText =
                            "UPDATE bakMakineler SET MakineAdi=@MakineAdi, BolumID=@BolumID, Secenek=@Secenek WHERE MakineID =" +
                            lblMakineID.Text;
        ...
        private void veriListele()
        {
            ...
            sqCom.CommandText =
                "SELECT Tbl1.MakineID, Tbl1.MakineAdi, Tbl2.Bolum, Tbl1.Secenek FROM bakMakineler Tbl1 LEFT JOIN bakBolumler Tbl2 ON Tbl1.BolumID=Tbl2.BolumID";
            sqCom.ExecuteNonQuery();
        ...
        private void dataGridView1_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.Rows.Count > 0)
                {
                   ...
                   // SqlDataAdapter da = new SqlDataAdapter();
                    sqCom.CommandText =
                        "SELECT Tbl1.MakineID, Tbl1.MakineAdi, Tbl2.Bolum, Tbl1.Secenek FROM bakMakineler Tbl1 LEFT JOIN bakBolumler Tbl2 ON Tbl1.BolumID=Tbl2.BolumID WHERE Tbl1.MakineID =" +
                        lblMakineID.Text;
                    sqCom.ExecuteNonQuery();
