...
namespace BakimTakipProgrami
{
    public partial class frmBolumler : Form
    {
            ...
            else if (btnKaydet.Text=="GÜNCELLE")
            {
                if (btnKaydet.Text=="GÜNCELLE" && txtBolumAdi.Text !="")
                {
                    if (!String.IsNullOrEmpty(lblBolumID.Text))
                    {
                        ...
                        SqlCommand sqCom = new SqlCommand();
                        sqCom.Connection = sqlConn;
                        sqCom.CommandText = "UPDATE bakBolumler SET Bolum=@Bolum, Pasif=@Pasif WHERE BolumID =" +
                                            lblBolumID.Text;
        ...
        private void veriListele()
        {
            ...
            SqlCommand sqCom = new SqlCommand();
            sqCom.Connection = sqlConn;
            //SqlDataAdapter da = new SqlDataAdapter(); //Fazladan kullanılmış
        ...
        private void dataGridView1_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.Rows.Count > 0)
                {
                    ...
                    sqCom.CommandText = "SELECT BolumID, Bolum, Pasif FROM bakBolumler WHERE BolumID =" +
                                        lblBolumID.Text;
                    sqCom.ExecuteNonQuery();
