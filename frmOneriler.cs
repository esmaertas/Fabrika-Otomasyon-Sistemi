    ...
    public partial class frmOneriler : Form
    {
        ...
        private void btnKaydet_Click(object sender, EventArgs e)
        {
            if (btnKaydet.Text=="KAYDET" && cmbBolum.SelectedIndex != -1 && cmbArizaTipi.SelectedIndex != -1 && cmbPersonel.SelectedIndex != -1 && cmbVardiya.SelectedIndex != -1 && txtOneri.Text !="" )
            {
                ...
                sqCom.CommandText =
                    "INSERT INTO bakOneriler (Tarih, Bolum, Personel, Vardiya, Oneri, Pasif) VALUES (@Tarih, @Bolum, @Personel, @Vardiya, @Oneri, @Pasif)";
            ...
            else if (btnKaydet.Text=="GÜNCELLE")
            {
                if (btnKaydet.Text == "GÜNCELLE" && cmbBolum.SelectedIndex != -1 && cmbArizaTipi.SelectedIndex != -1 && cmbPersonel.SelectedIndex != -1 && cmbVardiya.SelectedIndex != -1 && txtOneri.Text !="") 
                {
                    if (!String.IsNullOrEmpty(lblOneriID.Text))
                    {
                        ...
                        sqCom.CommandText =
                            "UPDATE bakOneriler SET Tarih=@Tarih, Bolum=@Bolum, Tur=@Tur, Personel=@Personel, Vardiya=@Vardiya, Oneri=@Oneri, Pasif=@Pasif WHERE OneriID =" +
                            lblOneriID.Text;
        ...
        private void veriListele()
        {
            ...
            sqCom.CommandText =
                @"SELECT Tbl1.OneriID, Tbl1.Tarih, Tbl2.Bolum, Tbl3.Turu, Tbl4.PersonelAdi, Tbl5.Vardiya, Tbl1.Oneri 
...
WHERE Tbl1.OneriID =" + lblOneriID.Text;
        ...
        private void LoadData(string kisim)
        {
            ...
            SqlCommand sc =
                new SqlCommand(
                    "SELECT * FROM bakPersoneller WHERE Pasif=0 AND Tur= '" + kisim + "'ORDER BY PersonelAdi ASC",
                    conn);
            SqlDataReader reader;
